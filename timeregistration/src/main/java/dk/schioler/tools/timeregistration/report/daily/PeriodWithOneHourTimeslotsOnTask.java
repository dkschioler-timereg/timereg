...
public class PeriodWithOneHourTimeslotsOnTask extends BasePeriod {

	public PeriodWithOneHourTimeslotsOnTask(Date start, Date end) {
		this(new DateTime(start), new DateTime(end));
	}

	public PeriodWithOneHourTimeslotsOnTask(DateTime start, DateTime end) {
		...
		while (start2.getMillis() < end2.getMillis()) {
			...
			Timeslot ts = new TimeslotImpl(start2, add1H, EventStats.EVENTCATEGORYPROVIDER_TASK);
			this.addTimeslot(ts);
			start2 = add1H;
		}
		Timeslot ts = new TimeslotImpl(start2, start2.plusHours(1), EventStats.EVENTCATEGORYPROVIDER_TASK);
		this.addTimeslot(ts);
